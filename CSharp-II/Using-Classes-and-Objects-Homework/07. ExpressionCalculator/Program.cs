...
namespace ExpressionCalculator
{
    class Program
    {
        ...
        static char[] arithmeticOperations = { '+', '-', '*', '/'};
        static string[] functions = { "pow", "ln", "sqrt" };

        static List<string> SeperateTokens(string input)
        {
        ...
        static int Presedence(string arithmeticOperator)
        {
        ...
        static Queue<string> ConvertToReversePolishNotation(List<string> tokens)
        {
            ...
            for (int i = 0; i < tokens.Count; i++)
            {
                ...
                {
                    // May have problems
                    while (stack.Count != 0 && arithmeticOperations.Contains(stack.Peek()[0]) && Presedence(currentToken) <= Presedence(stack.Peek()))
                    {
                        queue.Enqueue(stack.Pop());
                    }
        ...
        static void Main()
        {
            ...
            do
            {
                ...
                try
                {
                    var seperatedTokens = SeperateTokens(input);
